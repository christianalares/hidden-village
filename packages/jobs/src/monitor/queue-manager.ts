...
export class QueueManager {
  ...
  private async getJobsByTimeRange(
    ...
    try {
      ...
      const jobIds = await client.zrangebyscore(queueKey, startTime, endTime, 'LIMIT', 0, limit)
      ...
      return jobs.filter((job): job is Job => job !== null && job !== undefined)
    } catch (_error) {
   ...
   */
  private countCache = new LRUCache<string, Awaited<ReturnType<Queue['getJobCounts']>>>({
  ...
  async getQuickCounts(): Promise<{
    ...
    return this.cached('quick-counts', 2000, async () => {
      ...
      totals.total =
        totals.waiting + totals.active + totals.completed + totals.failed + totals.delayed
  ...
  async getOverview(): Promise<OverviewStats> {
    return this.cached('overview', this.CACHE_TTL.overview, async () => {
      ...
      const completedToday = queues.reduce((sum, q) => sum + q.counts.completed, 0)
      ...
      return this.withTimeout(
        (async () => {
          ...
          const createEmptyBuckets = (): HourlyBucket[] => {
            ...
            const startHour = Math.floor(twentyFourHoursAgo / (60 * 60 * 1000)) * (60 * 60 * 1000)
            for (let i = 0; i < 24; i++) {
              buckets.push({
          ...
          for (const { queueName, completedJobs, failedJobs } of queueResults) {
            ...
            for (const job of completedJobs) {
              if (!job?.finishedOn || job.finishedOn < twentyFourHoursAgo) continue
          ...
          const totalCompleted = aggregateBuckets.reduce((sum, b) => sum + b.completed, 0)
    ...
    return this.cached('activity', this.CACHE_TTL.activity, async () => {
      ...
      for (const { completedJobs, failedJobs } of queueResults) {
        ...
        for (const job of completedJobs) {
          ...
          const bucketIndex = Math.floor((job.finishedOn - startTime) / bucketSize)
  ...
  async getJobs(
    ...
    for (const type of types) {
      ...
      jobsWithState.push(...typeJobs.map((job) => ({ job, state: type as JobStatus })))
   ...
   */
  private jobMatchesFilters(job: Job, filters: Record<string, string>): boolean {
    if (!job.data || typeof job.data !== 'object') {
          ...
          types.map(async (type) => {
            try {
              ...
              for (const job of jobs) {
                ...
                matches.push({
                  queue: queueName,
                  job: (await this.jobToInfo(job, 'full', type as JobStatus)) as JobInfo,
                })
              }
   ...
   */
  async cleanJobs(queueName: string, status: 'completed' | 'failed', grace = 0): Promise<number> {
    const queue = this.queues.get(queueName)
    if (!queue) return 0
  ...
  private async getLatestRuns(
    ...
    const allTypes: JobStatus[] = ['waiting', 'active', 'completed', 'failed', 'delayed']
  ...
  async getAllRuns(
    ...
    const baseFetchPerQueue = Math.max(Math.ceil((limit * 2) / numQueues) + 3, 5)

    let allJobs: { job: Job; queueName: string; state: JobStatus }[] = []

    // Helper function to fetch from a single queue
    const fetchFromQueue = async (queueName: string, queue: Queue, fetchCount: number) => {
      ...
      if (hasTimeRange && filters?.timeRange) {
        ...
        const otherTypes = types.filter((t) => t !== 'completed' && t !== 'failed')
    ...
    const results = await Promise.all(
      queueEntries.map(([queueName, queue]) => fetchFromQueue(queueName, queue, baseFetchPerQueue)),
    )
    allJobs = results.flat()

    // Apply filters BEFORE sorting and pagination
    if (filters) {
      allJobs = allJobs.filter(({ job }) => this.jobMatchesAllFilters(job, filters))
    }
   ...
   */
  async getTagValues(field: string, limit = 50): Promise<{ value: string; count: number }[]> {
    ...
    const queueResults = await Promise.all(
      queueEntries.map(async ([, queue]) => {
        const jobArrays = await Promise.all(types.map((type) => queue.getJobs(type as any, 0, 100)))
   ...
   */
  private getSortValue(item: JobInfo | RunInfo, field: string): string | number {
    switch (field) {
      case 'timestamp':
   ...
   */
  private getSortValueForList(item: RunInfoList, field: string): string | number {
    switch (field) {
      case 'timestamp':
        return item.timestamp ?? 0
      case 'name':
   ...
   */
  private getSchedulerSortValue(item: SchedulerInfo, field: string): string | number {
    switch (field) {
      case 'name':
        return item.name.toLowerCase()
      case 'queueName':
   ...
   */
  private getDelayedSortValue(item: DelayedJobInfo, field: string): string | number {
    switch (field) {
      case 'name':
        return item.name.toLowerCase()
      case 'queueName':
      ...
      const queueResults = await Promise.all(
        relevantQueues.map(async ({ queueName, queue }) => {
          try {
            ...
            const waitingChildrenJobs = await queue.getJobs(['waiting-children'], 0, 50)
            ...
            const otherTypes = ['waiting', 'active', 'completed', 'failed', 'delayed']
            const otherJobArrays = await Promise.all(
              otherTypes.map(async (type) => {
                try {
   ...
   */
  private buildFlowJob(request: CreateFlowRequest | CreateFlowChildRequest): any {
    const result: any = {
      name: request.name,
    ...
    if (request.children && request.children.length > 0) {
      result.children = request.children.map((child) => this.buildFlowJob(child))
    }
